  ...
  const generateSuggestions = (currentUsername: string): string[] => {
    ...
    if (clean.length > 3) {
      ...
      suggestions.add(
        `${clean
          .split('')
          .map(c => (vowels.includes(c) ? pick(vowels.split('')) : c))
          .join('')}${randNum(1)}`,
      ); // Vowel swap
    }
    ...
    while (
      ...
      suggestions.size > finalSuggestions.length
    ) {
      const nextSuggestion = Array.from(suggestions).find(
        s =>
          !finalSuggestions.includes(s) &&
          s.toLowerCase() !== currentUsername.toLowerCase(),
      );
      if (nextSuggestion) {
    ...
    if (finalSuggestions.length < 3) {
      ...
      fallback.forEach(fb => {
        if (
          finalSuggestions.length < 3 &&
          !finalSuggestions.includes(fb) &&
          fb.toLowerCase() !== currentUsername.toLowerCase()
        ) {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="w-full">
            ...
            {user?.username && !user?.is_anonymous && (
              <div className="mt-2 border-t px-4 py-6">
                <h5 className="mb-3 flex items-center gap-1.5 text-base font-semibold">
                  <Sparkles className="h-4 w-4 text-yellow-500" />
                  Need inspiration?
                </h5>
                <div className="flex flex-wrap gap-2">
                  {generateSuggestions(user.username).map(
                    (suggestion, index) => (
                      <button
                        key={index}
                        type="button"
                        className="cursor-pointer rounded-full bg-neutral-100 px-3 py-1.5 text-sm transition-colors hover:bg-neutral-200 focus:outline-none focus:ring-2 focus:ring-blue focus:ring-opacity-50 dark:bg-neutral-800 dark:hover:bg-neutral-700"
                        onClick={() => {
