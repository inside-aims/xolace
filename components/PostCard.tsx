...
const PostCard: React.FC<PostCardProps> = ({ post, className }) => {
  ...
  return (
    <Card className={`w-full mb-5 overflow-hidden transition-colors duration-300 ${className}`}>
      <CardHeader className="flex flex-row justify-between items-start p-4">
        <div className="flex gap-4 items-center">
          ...
          <div className="flex flex-col">
            ...
            <small className="text-xs text-gray-500 dark:text-gray-400">{post.timestamp}</small>
          </div>
        </div>
      ...
      <CardFooter className="flex justify-between items-center p-4">
        <div className="flex items-center space-x-2">
          <div className="flex items-center">
            <UpvoteIcon filled={voteStatus === 'upvote'} onClick={handleUpvote} />
            <span className="text-sm ml-1">{upvoteCount}</span>
          </div>
          <div className="flex items-center">
            <DownvoteIcon filled={voteStatus === 'downvote'} onClick={handleDownvote} />
