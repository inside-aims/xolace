...
const HomeContent = ({  onNavigate }: { onNavigate: (route: string) => void; }) => {
  ...
  return (
    <div className="flex flex-col gap-8 min-h-screen">
      <div className="bg-white shadow-sm border text-neutral-700 dark:bg-neutral-800 dark:text-white rounded-2xl p-4 flex-1 items-start justify-center flex flex-col gap-8">
...
const ActionCard = ({ title, description, onClick, icon, color }: ActionProps) => {
  return (
    <div className={`${color} rounded-2xl p-6 flex flex-col justify-between h-60 shadow-md hover:scale-105 transition-transform cursor-pointer text-white`}
      onClick={onClick}>
