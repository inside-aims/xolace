...
import { CampfireMember } from '../campfires/useCampfireMembers';

/**
 * A React Query hook to efficiently fetch a batch of signed URLs for post avatars.
 * @param profiles - The array of profiles from the initial feed query.
 * @returns The result of the React Query operation, containing a map of avatar paths to signed URLs.
 */
export const getSignedProfileAvatarUrls = (profiles: CampfireMember[] | undefined) => {
  ...
  const avatarPaths = useMemo(() => {
    ...
    return profiles
      .filter(
        profile =>
          // Check if the URL is a path, not a full http URL
          profile.avatar_url &&
          !profile.avatar_url.startsWith('http'),
      )
      .map(profile => profile.avatar_url!); // The '!' is safe due to the filter
  }, [profiles]);
