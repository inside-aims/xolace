...
export function usePostMutations() {
  ...
  const deletePostMutation = useMutation({
    mutationFn: async ({ postId, postCreatedBy, content }: { postId: string; postCreatedBy: string | undefined; content: string | undefined }) => {
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: async ({ commentId, postId, postCreatedBy, content, userId }: { commentId: number; postId: string | undefined; postCreatedBy: string | undefined; content: string | undefined; userId: string | undefined }) => {
      await supabase
        .from('comments')
        .delete()
        .eq('id', commentId);
